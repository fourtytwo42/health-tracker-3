...
const formatIngredientAmount = (amount: number, unit: string, ingredientName: string, useMetricUnits: boolean = false): string => {
  const unitLower = unit.toLowerCase();
  const ingredientLower = ingredientName.toLowerCase();
  
  // If metric units are enabled, return the original metric values
  if (useMetricUnits) {
    if (unitLower === 'g') {
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: '8px', color: '#6c757d', fontWeight: 'bold' }}>TIME</div>
                  <div style={{ fontSize: '12px', color: '#333', fontWeight: 'bold' }}>
                    {recipe.totalTime || ((recipe.prepTime || 0) + (recipe.cookTime || 0)) || 45}m
                  </div>
                </div>
              </div>
            </div>
