...
function processBrandedFood(food) {
  ...
  const ingredientData = {
    ...
    water: extractNutrientValue(nutrients, 1051),
    calcium: extractNutrientValue(nutrients, 1087),
    potassium: extractNutrientValue(nutrients, 1092),
...
async function seedAllUSDAData() {
  ...
  try {
    ...
    console.log('Processing branded foods...');
    const brandedData = JSON.parse(fs.readFileSync(path.join(__dirname, '../Data/FoodData_Central_branded_food_json_2025-04-24.json'), 'utf8'));
    const brandedFoods = brandedData.Foods || [];
    
    for (const food of brandedFoods.slice(0, 1000)) { // Limit to first 1000 for performance
      totalProcessed++;
      try {
        const ingredientData = processBrandedFood(food);
        ...
        if (totalAdded % 100 === 0) {
          console.log(`Added ${totalAdded} ingredients...`);
        }
      } catch (error) {
        console.error(`Error processing branded food: ${error.message}`);
      }
    }
    ...
    for (const food of legacyFoods.slice(0, 1000)) { // Limit to first 1000 for performance
      totalProcessed++;
      try {
    ...
    for (const food of surveyFoods.slice(0, 1000)) { // Limit to first 1000 for performance
      totalProcessed++;
      try {
