...
async function processUsdaFile(filePath, fileType, maxItems = 1000) {
  ...
  try {
    ...
    while (processed < maxItems) {
      ...
      // Find food objects in this chunk
      const foodObjects = extractFoodObjects(chunk);
      
      for (const foodJson of foodObjects) {
...
function extractFoodObjects(chunk) {
  const objects = [];
  let braceCount = 0;
  let startIndex = -1;
  let inString = false;
    ...
    if (!inString) {
      if (char === '{') {
        ...
        if (braceCount === 0 && startIndex !== -1) {
          const objectJson = chunk.substring(startIndex, i + 1);
          // Check if this looks like a food object
          if (objectJson.includes('"description"') && objectJson.includes('"foodNutrients"')) {
...
async function seedAllUsdaIngredients() {
  ...
  for (const file of files) {
    if (fs.existsSync(file.path)) {
      const result = await processUsdaFile(file.path, file.type, 1000); // Process 1000 items per file
