...
async function seedIngredients() {
  console.log('Seeding ingredients from USDA data...');
  
  const filePath = path.join(process.cwd(), 'Data', 'FoodData_Central_sr_legacy_food_json_2018-04.json');
  
  if (!fs.existsSync(filePath)) {
    console.log('USDA data file not found, skipping ingredients seeding');
    return;
  }
  ...
  try {
    ...
    for (const food of foods) {
      try {
        ...
        // Map nutrients to our fields
        const ingredient = {
          name: food.description || 'Unknown Food',
          description: food.description || '',
          servingSize: food.servingSize || '100g',
          calories: nutrients[1008] || 0, // Energy (kcal)
          protein: nutrients[1003] || 0, // Protein (g)
          carbs: nutrients[1005] || 0, // Carbohydrate (g)
          fat: nutrients[1004] || 0, // Total lipid (fat) (g)
          fiber: nutrients[1079] || 0, // Fiber (g)
          sugar: nutrients[2000] || 0, // Sugars (g)
          sodium: nutrients[1093] || 0, // Sodium (mg)
          cholesterol: nutrients[1253] || 0, // Cholesterol (mg)
          saturatedFat: nutrients[1258] || 0, // Fatty acids, total saturated (g)
          monounsaturatedFat: nutrients[1292] || 0, // Fatty acids, total monounsaturated (g)
          polyunsaturatedFat: nutrients[1293] || 0, // Fatty acids, total polyunsaturated (g)
          transFat: nutrients[1257] || 0, // Fatty acids, total trans (g)
          netCarbs: (nutrients[1005] || 0) - (nutrients[1079] || 0), // Net carbs
          category: food.foodCategory?.description || 'Other',
          aisle: 'Pantry', // Default aisle
          isActive: true
        };
        
        // Fix serving size if it contains "undetermined"
        if (ingredient.servingSize.includes('undetermined')) {
        ...
        // Skip if no calories or invalid data
        if (ingredient.calories <= 0 || !ingredient.name || ingredient.name === 'Unknown Food') {
          skipped++;
          continue;
        }
...
async function seedExercises() {
  ...
  if (!fs.existsSync(filePath)) {
    console.log('Exercise data file not found, skipping exercises seeding');
    return;
  }
...
async function main() {
  ...
  try {
    ...
    // Seed ingredients and exercises
