...
async function processUsdaFile(filePath, fileType, maxItems = 200) {
  ...
  try {
    ...
    // For large files, we'll use a line-by-line approach
    ...
    return new Promise((resolve, reject) => {
      readStream.on('data', (chunk) => {
        ...
        // Process the buffer to find complete food objects
...
async function seedAllUsdaIngredients() {
  ...
  for (const file of files) {
    if (fs.existsSync(file.path)) {
      try {
        const result = await processUsdaFile(file.path, file.type, 200); // Process 200 items per file
