...
async function seedSampleBrandedFoods() {
  try {
    console.log('🌱 Starting USDA sample branded foods seeding...\n');
    
    const filePath = path.join(__dirname, '../ingredientData/sample-branded-foods.json');
    
    if (!fs.existsSync(filePath)) {
      console.error('❌ Sample branded foods file not found:', filePath);
      return;
    }

    const stats = fs.statSync(filePath);
    console.log(`📖 File size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
    console.log('📖 Reading sample branded foods file...\n');
    ...
    console.log(`📊 Found ${foods.length} branded foods to process\n`);
    ...
    for (const foodItem of foods) {
      try {
        ...
        if (processedCount % 10 === 0) {
          console.log(`⏳ Processed ${processedCount}/${foods.length} items... (Added: ${addedCount}, Skipped: ${skippedCount})`);
        }
    ...
    console.log(`\n✅ Sample branded foods seeding completed!`);
    console.log(`📊 Summary:`);
    ...
    console.error('❌ Error seeding sample branded foods:', error);
  } finally {
