...
async function processUsdaFileStreaming(filePath, fileType, maxItems = 5000) {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      readStream.on('data', (chunk) => {
        ...
        while (true) {
          // Find the start of a food object
          const startIndex = buffer.indexOf('{"foodClass"');
          ...
          if (braceCount === 0) {
            ...
            const objectJson = buffer.substring(startIndex, endIndex);
            try {
...
async function seedAllUsdaIngredients() {
  ...
  for (const file of files) {
    if (fs.existsSync(file.path)) {
      try {
        const result = await processUsdaFileStreaming(file.path, file.type, 2000); // Process 2000 items per file
        totalProcessed += result.processed;
        totalSkipped += result.skipped;
        totalErrors += result.errors;
        
        // Insert processed ingredients into database
        if (result.processed > 0) {
          console.log(`💾 Inserting ${result.processed} ingredients into database...`);
          // Note: In a real implementation, you'd batch insert these
